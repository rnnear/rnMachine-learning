...
public:
	...
	static rnmatrix transposition(const rnmatrix& rnmat);
	rnmatrix(){}
	...
	explicit rnmatrix(const std::vector<data_type>& rnvec): rowDim(1), colDim(rnvec.size()), mat(1, rnvec){}
	explicit rnmatrix(const std::vector<std::vector<data_type> >& rnvvec): rowDim(rnvvec.size()), colDim((rnvvec.size() == 0?0:rnvvec[0].size())),
		mat(rnvvec){} //exception
	rnmatrix(const std::vector<data_type>& rnvec, int); //column vector matrix
	...
	void transposition();
	void calcEach(const std::function<T(const T)>& rnf);
	void display()const;
...
template<typename T>
void rnmatrix<T>::calcEach(const std::function<T(const T)>& rnf)
{
	for(size_type rnrow = 0; rnrow < rowDim; ++rnrow)
		for(size_type rncol = 0; rncol < colDim; ++rncol)
			mat[rnrow][rncol] = rnf(mat[rnrow][rncol]);
}
template<typename T>
rnmatrix<T> rnmatrix<T>::operator ^(const rnmatrix& rnright)const
{
//	if(rowDim != rnright.getRowDim()||colDim != rnright.getColDim())
//		THROW std::runtime_error("operator^: dimension mismatch");
	return dotTime(*this, rnright);
}
template<typename T>
...
template<typename T>
rnmatrix<T> rnmatrix<T>::transposition(const rnmatrix& rnmat)
{
